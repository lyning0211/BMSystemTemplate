...
//使用Enter键快捷操作
$(document).keydown(function (event) { if (event.keyCode === 13) { SearchAllInfo() } });

//js日期格式转换
function ConvertDate(date) {
    if (date != null && date != "") {
        var newDate = eval('new ' + (date.replace(/\//g, '')));
        //console.log(date1);
        ...
        var logintime = year + "-" + month + "-" + day + " " + hour + ":" + minutes + ":" + second + "";
...
function AccountInfo(UserID) {
    $.ajax({
        type: "POST",
        url: "/Helper/GetAccountsInfo",
        contentType: 'application/x-www-form-urlencoded;charset=utf-8',
        data: { userID: UserID },
        dataType: "text",
        success: function (data) {
            result = data;
            window.sessionStorage.setItem("UserInfo", result);
        },
        error: function (e) {
            console.log(e);
        }
    });
    var result = window.sessionStorage.getItem("UserInfo");
    return result;
}