...
namespace Web.Controllers
{
    ...
    public class SystemManageController : HelperController
    {
        ...
        /// <returns></returns>
        [FilterConfig]
        public JsonResult GetConfigTableData(string jsonData)
        {
            ...
            try
            {
                ...
                string myWhere = BuildWhereConfigTableData(pDict["currentwhere"].ToString());
                string orderBy = "Create_Time DESC";
                DataTable dt = SystemManageDAL.GetParameterTable(myWhere, orderBy);
                if (dt.Rows.Count == 0)
                {
        ...
        /// <returns></returns>
        [FilterConfig]
        private string BuildWhereConfigTableData(string jsonWhere)
        {
        ...
        /// <returns></returns>
        [FilterConfig]
        public JsonResult GetConfigInfoData(string parameterName)
        {
            Message message = new Message();
            try
            {
                string myWhere = "ParameterName='" + parameterName + "'";
                DataTable dt = SystemManageDAL.GetParameterTable(myWhere, "");
                if (dt.Rows.Count > 0)
                {
                    message.Success = true;
        ...
        /// <returns></returns>
        [FilterConfig]
        public JsonResult SaveConfigInfoData(string jsonData)
        {
            Message message = new Message();
            try
            {
                ...
                //保存数据
                string result = SystemManageDAL.SaveConfigInfoData(pDict, pUserMapInfo);
                if (result == "")
                {
                    message.Success = true;
        ...
        /// <returns></returns>
        [FilterConfig]
        public JsonResult DelConfigInfoData(string id)
        {
            Message message = new Message();
            try
            {
                //获取缓存登录用户信息
                pUserMapInfo = (LoginUserModel)Session["LoginUser"];
                string result = SystemManageDAL.DelConfigInfo(id, pUserMapInfo);
                if (result == "")
                {
                    message.Success = true;
        ...
        /// <returns></returns>
        [FilterConfig]
        public JsonResult DeleteConfigInfoData(string idList)
        {
            Message message = new Message();
            try
            {
                //获取缓存登录用户信息
                pUserMapInfo = (LoginUserModel)Session["LoginUser"];
                string result = SystemManageDAL.DeleteConfigInfo(idList, pUserMapInfo);
                if (result == "")
                {
                    message.Success = true;
                    message.ReturnString = "删除成功";
