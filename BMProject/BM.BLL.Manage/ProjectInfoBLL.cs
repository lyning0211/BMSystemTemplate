        ...
        public static string SaveEducationalBackgroundInfoData(Dictionary<string, object> dictData, LoginUserModel pUserMapInfo)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(dictData["EducationalBackgroundGUID"].ToString()))
                {
                    ...
                    string newGUID = Guid.NewGuid().ToString();
                    //写入痕迹日志
                    sqlStr.AppendFormat("INSERT INTO dbo.Log_Trace (Object_Type,Object_GUID,Object_Name,Object_Field,Old_Value,New_Value,UserID,UserName,UserRoleID,UserRoleName,Operation_Time,Operation_Type,Operation_IP,Operation_Note) VALUES('educationalbackground','" + newGUID + "',@EducationalBackgroundName,'','','','" + pUserMapInfo.UserID + "','" + pUserMapInfo.UserName + "','" + pUserMapInfo.UserRoleID + "','" + pUserMapInfo.UserRoleName + "','" + mydate + "','add','" + pUserMapInfo.LoginIP + "','添加学历信息');");

                    //添加数据
                    sqlStr.AppendFormat("INSERT INTO dbo.SA_EducationalBackground (EducationalBackgroundGUID,EducationalBackgroundName,Notes,Create_Time) VALUES ('" + newGUID + "',@EducationalBackgroundName,@Notes,GETDATE());");
                }
                else
                {
                    ...
                    foreach (var item in dictData)
                    {
                        ...
                        {
                            //写入痕迹日志
                            sqlStr.AppendFormat("INSERT INTO dbo.Log_Trace (Object_Type,Object_GUID,Object_Name,Object_Field,Old_Value,New_Value,UserID,UserName,UserRoleID,UserRoleName,Operation_Time,Operation_Type,Operation_IP,Operation_Note) VALUES('educationalbackground',@EducationalBackgroundGUID,@EducationalBackgroundName,'" + object_name + "','" + old_value + "','" + new_value + "','" + pUserMapInfo.UserID + "','" + pUserMapInfo.UserName + "','" + pUserMapInfo.UserRoleID + "','" + pUserMapInfo.UserRoleName + "','" + mydate + "','update','" + pUserMapInfo.LoginIP + "','修改学历信息');");
                        }
                    }
                    //修改数据
                    sqlStr.AppendFormat("UPDATE dbo.SA_EducationalBackground SET EducationalBackgroundName=@EducationalBackgroundName,Notes=@Notes WHERE EducationalBackgroundGUID=@EducationalBackgroundGUID;");
                }
